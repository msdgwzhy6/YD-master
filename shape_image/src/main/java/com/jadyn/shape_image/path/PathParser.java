package com.jadyn.shape_image.path;
...
public class PathParser {
     ...
     */
    public static void parse(String pathString, Path p,double mu) {
        ...
        while (ph.pos < n) {
            ...
            //因为本地的图形path字符串，偏小，所以得到的每个数字都乘以3
            switch (cmd) {
                case 'M':
                case 'm': {
                    float x = ph.nextFloat()*(float) mu;
                    Log.i("心", "x---"+x);
                    float y = ph.nextFloat()*(float) mu;
                    Log.i("心", "y---"+y);
                    if (cmd == 'm') {
                        subPathStartX += x;
                        Log.i("心", "subPathStartX---"+subPathStartX);
                        subPathStartY += y;
                        p.rMoveTo(x, y);
                        lastX += x;
                        Log.i("心", "lastX---"+lastX);
                ...
                case 'c': {
                    wasCurve = true;
                    float x1 = ph.nextFloat()*(float) mu;
                    float y1 = ph.nextFloat()*(float) mu;
                    float x2 = ph.nextFloat()*(float) mu;
                    float y2 = ph.nextFloat()*(float) mu;
                    float x = ph.nextFloat()*(float) mu;
                    float y = ph.nextFloat()*(float) mu;
                    if (cmd == 'c') {
                        x1 += lastX;
                ...
                case 'A':
                case 'a': {
                    float rx = ph.nextFloat()*(float) mu;
                    
                    float ry = ph.nextFloat()*(float) mu;
                    float theta = ph.nextFloat()*(float) mu;
                    int largeArc = (int) ph.nextFloat()*(int) mu;
                    int sweepArc = (int) ph.nextFloat()*(int) mu;
                    float x = ph.nextFloat();
