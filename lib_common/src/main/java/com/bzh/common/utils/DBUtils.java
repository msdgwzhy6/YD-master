...
public class DBUtils {
    ...
    public void insertHasRead(String table, String key, int value) {
        ...
        if (cursor.getCount() > 200) {
            if (cursor.moveToNext())
                mSQLiteDatabase.delete(table, "id=?", new String[] {String.valueOf(cursor.getInt(cursor
                        .getColumnIndex("id")))});
        }
        cursor.close();
    ...
    public class DBHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL("create table if not exists " + Config.GUOKR + "(id integer  primary key autoincrement,key " +
                    "text unique,is_read integer);");
            db.execSQL("create table if not exists " + Config.IT + "(id integer primary key autoincrement,key text " +
                    "unique,is_read integer)");
            db.execSQL("create table if not exists " + Config.VIDEO + "(id integer primary key autoincrement,key text" +
                    " unique,is_read integer)");
            db.execSQL("create table if not exists " + Config.ZHIHU + "(id integer primary key autoincrement,key text" +
                    " unique,is_read integer)");
            db.execSQL("create table if not exists " + Config.WEIXIN + "(id integer primary key autoincrement,key " +
                    "text unique,is_read integer)");
        }
